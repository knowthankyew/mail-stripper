...
5. 🤖 **Optional ML Integration**: Works 100% standalone out-of-the-box with high-speed extractive summarization, and seamlessly pairs with the public [`event-driven-ftaas`](https://github.com/knowthankyew/event-driven-ftaas) microservice (`SmolLM2-135M` on `http://localhost:8000`) for generative understanding when available. *(See [Optional: Enable Generative Summaries](#-optional-enable-generative-summaries) for setup.)*
...
## 🤖 Optional: Enable Generative Summaries

`mailStripper` works 100% standalone out of the box, but you can optionally pair it with a lightweight sibling microservice for generative AI understanding:

- **What It Is**: [`event-driven-ftaas`](https://github.com/knowthankyew/event-driven-ftaas) is a lightweight, local LLM microservice (`SmolLM2-135M`) from the same author that provides generative one-sentence titles — as opposed to the default fast extractive mode.
- **How to Start It**: Follow the setup instructions at **[`event-driven-ftaas`](https://github.com/knowthankyew/event-driven-ftaas)** so the service is running and reachable at `http://localhost:8000`.
- **Zero-Config Detection**: No configuration is required in `mailStripper`. On each email submission, `MlHybridSummarizer` probes `http://localhost:8000/healthz` with a 1.2-second timeout. If the health check succeeds, it calls `http://localhost:8000/api/v1/inference/generate` to produce a generative one-sentence title. If the probe fails or times out, it silently falls back to the extractive engine — the user sees no error.
- **What Changes in the Output**:
  - **Without `event-driven-ftaas`**: The one-sentence title is produced by the fast, rule-based extractive engine (MimeKit-powered), 100% offline.
  - **With `event-driven-ftaas` running**: The one-sentence title is silently replaced with a `SmolLM2-135M`-generated sentence. The summary bullets remain extractive. There is no visible badge or mode indicator in the UI — the improvement is in output quality only.
