...
`mailStripper` works 100% standalone out of the box with fast, offline extractive summarization. You can optionally pair it with a sibling microservice for generative AI summaries:

- **What It Is**: [`event-driven-ftaas`](https://github.com/knowthankyew/event-driven-ftaas) is a lightweight, local LLM microservice (`SmolLM2-135M`) created by the same author. It generates synthesized, intent-aware one-sentence titles and summary bullets rather than extracting raw sentences from the email body.
- **How to Start It**: Follow the setup instructions at **[`event-driven-ftaas`](https://github.com/knowthankyew/event-driven-ftaas)** to run the service locally so it is reachable at `http://localhost:8000`.
- **Zero-Config Auto-Detection**: No configuration or API keys required in `mailStripper`. When processing an email, `mailStripper` automatically probes `http://localhost:8000`. If the microservice is reachable, generative mode activates automatically; if it is stopped or unavailable, it silently falls back to extractive mode.
- **UI Experience Difference**:
  - **Without `event-driven-ftaas` (Default Extractive)**: Fast, deterministic rule-based extraction. The header status indicator displays `Summarizer: Smart Extractive` and the result badge shows `Smart Extractive`.
  - **With `event-driven-ftaas` (Generative Mode)**: Titles and summaries are generated by `SmolLM2-135M`. The header displays a green active dot with `ML: SmolLM2 Active`, and the result badge indicates `SmolLM2-135M (FTaaS ML + MimeKit)`.
